...
async function handler(
  ...
  console.log(`📊 Dependencies ${method} request for team: ${teamSlug}`);
  console.log(`🔗 Backend URL: ${BACKEND_URL}/teams/${teamSlug}/dependencies`);

  try {
    // Parse body untuk POST dan DELETE (ElysiaJS supports body in DELETE)
    ...
    const res = await fetch(`${BACKEND_URL}/teams/${teamSlug}/dependencies`, {
      method: method,
      headers: {
        ...forwardAuth(req),
    ...
    console.error("❌ Dependencies proxy error:", errorMessage);
